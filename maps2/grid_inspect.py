...
grid1, _ = gcpy.grid.make_grid_SG(60, 3, 261.0, 36.0)
grid2, _ = gcpy.grid.make_grid_SG(180, 1, 170, -90)
...
plt.title('C180-global', fontsize=10)
...
plt.title('C180e-US', fontsize=10)
...
plt.colorbar(pcm, cax=ax, ticks=[40, 55, 70], label='Resolution (km)', extend='max')
...
for nf in range(6):
    ...
    draw_major_grid_boxes(ax, grid1_xe[nf,...], grid1_ye[nf,...], transform=ccrs.PlateCarree(), color=cmap(3), zorder=101)
    draw_major_grid_boxes(ax, grid2_xe[nf,...], grid2_ye[nf,...], transform=ccrs.PlateCarree(), color=cmap(0), zorder=102)

# ax.contour(xx1, yy1, cc1, transform=ccrs.PlateCarree(), levels=[100, 200, 400], colors=cmap(3), linestyles='dashed')
ax.contour(xx, yy, res_c, transform=ccrs.PlateCarree(), levels=[100, 250, 500], colors=cmap(0), linestyles=['dashed', 'dashdot', 'dotted'], linewidths=[0.9, 0.9, 0.9], inlines=True)
...
ax.annotate('100 km', xy=ccrs.EqualEarth().transform_point(-92.89, 1.21, ccrs.PlateCarree()), xycoords='data', color=cmap(0),
            xytext=(0.25, 0.35), textcoords='axes fraction', arrowprops=dict(arrowstyle="->", edgecolor=cmap(0)), horizontalalignment='center', verticalalignment='center')

ax.annotate('250 km', xy=ccrs.EqualEarth().transform_point(-60.79, -50.59, ccrs.PlateCarree()), xycoords='data', color=cmap(0),
            xytext=(0.5, 0.05), textcoords='axes fraction', arrowprops=dict(arrowstyle="->", edgecolor=cmap(0)), horizontalalignment='center', verticalalignment='center')

ax.annotate('500 km', xy=ccrs.EqualEarth().transform_point(80, -2, ccrs.PlateCarree()), xycoords='data', color=cmap(0),
            xytext=(0.7, 0.65), textcoords='axes fraction', arrowprops=dict(arrowstyle="->", edgecolor=cmap(0)), horizontalalignment='center', verticalalignment='center')


legend = ax.legend(custom_lines, ['C180-global', 'C180e-US', '125 km resolution', '250 km resolution', '500 km resolution'], loc='upper right')
legend.set_zorder(200)

plt.tight_layout()
#plt.savefig('/home/liam/gmd-sg-manuscript-2020/figures/new2.png', dpi=300, pad_inches=0.01)
plt.show()
